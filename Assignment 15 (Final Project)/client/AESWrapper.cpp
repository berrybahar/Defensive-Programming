...
AESWrapper::AESWrapper(const std::string& key)
{
	if (key.size() != DEFAULT_KEYLENGTH)
		throw std::length_error("key length must be 32 bytes for AES-256");

	// Copy the key from the string to the _key array
	memcpy_s(_key, DEFAULT_KEYLENGTH, key.data(), DEFAULT_KEYLENGTH);
}
